...
ROLE_CATALOG: tuple[RoleDefinition, ...] = (
    ...
    RoleDefinition(
        ...
        "افسونگر",
        RoleTeam.MAFIA,
        "هر شب یک بازیکن را افسون می‌کند تا روز بعد نتواند صحبت کند.",
        objective="کمک به پیروزی مافیا با ساکت‌کردن و خنثی‌کردن نقش‌های کلیدی شهر.",
        night_ability="هر شب یک بازیکن را افسون می‌کند؛ آن بازیکن روز بعد حق صحبت و دفاع ندارد (سکوت).",
        timing="هر شب یک‌بار، همراه با تیم مافیا.",
        limitations="معمولاً نمی‌تواند دو شب پیاپی یک نفر را هدف بگیرد؛ افسون فقط برای یک روز اثر دارد.",
        interactions="با ساکت‌کردن نقش‌های مؤثر شهر (کارآگاه، دکتر، شهردار) قدرت دفاعی شهر را می‌شکند.",
        strategy="بازیکنی را افسون کن که آن روز می‌خواهد اطلاعات مهمی بدهد یا از خودش دفاع کند.",
    ),
    RoleDefinition(
        RoleCode.NATO,
        "ناتو",
        RoleTeam.MAFIA,
        "می‌تواند با اعلام آتش‌بس، شلیک‌های آن شب را خنثی کند تا مافیا زنده بماند.",
        objective="حفظ اعضای مافیا با بی‌اثر کردن شلیک‌ها در شب‌های خطرناک، به سود تیم مافیا.",
        night_ability="چند بار محدود در بازی می‌تواند «آتش‌بس/حالت دفاعی» اعلام کند تا هیچ شلیکی آن شب اثر نکند.",
        timing="تعداد استفاده محدود در کل بازی، هماهنگ با تیم مافیا.",
        limitations="پس از پایان دفعات مجاز دیگر قدرتی ندارد؛ آتش‌بس شلیک همه (از جمله شهر) را خنثی می‌کند.",
        interactions="می‌تواند تیر تک‌تیرانداز یا حمله‌ی نقش‌های شهر علیه مافیا را در همان شب بی‌اثر کند.",
        strategy="قدرتت را برای شبی نگه دار که احتمال لو رفتن یا هدف‌گیری مافیا بالاست.",
    ),
    ...
    # --- Capo (تفنگدار کاپو) city-side special roles ---
    RoleDefinition(
        RoleCode.SUSPECT,
        "مظنون",
        RoleTeam.CITIZEN,
        "شهروندی که استعلامش برای کارآگاه مثبت (مافیا) دیده می‌شود.",
        objective="کمک به شهر با وجود ظاهر مشکوک؛ اثبات بی‌گناهی و حذف مافیا.",
        night_ability="اکشن شبانه ندارد؛ نقشش انفعالی است.",
        day_ability="مثل شهروند بحث و رأی می‌دهد اما باید ظنّ روی خودش را مدیریت کند.",
        timing="در طول روزها.",
        limitations=(
            "استعلام کارآگاه روی او «مثبت» است؛ هنگام خروج از بازی گرداننده او را "
            "«شهروند» اعلام می‌کند. قابلیت سوداگری‌شدن (جذب‌شدن به مافیا) دارد."
        ),
        interactions="کارآگاه او را مافیا می‌بیند؛ دن مافیا می‌تواند او را سوداگری کند.",
        strategy="زود و شفاف بازی کن تا با وجود استعلام مثبت، اعتماد شهر را جلب کنی.",
    ),
    RoleDefinition(
        RoleCode.ARMORSMITH,
        "زره‌ساز",
        RoleTeam.CITIZEN,
        "هر شب به یک نفر زره می‌دهد (شبیه دکتر)؛ فقط یک‌بار می‌تواند خودش را زره کند.",
        objective="محافظت از نقش‌های کلیدی شهر با زره در برابر شلیک.",
        night_ability="هر شب یک بازیکن را انتخاب و به او زره می‌دهد تا یک حمله را دفع کند.",
        timing="هر شب یک‌بار.",
        limitations="فقط یک‌بار در کل بازی می‌تواند روی خودش زره بگذارد.",
        interactions=(
            "اگر جادوگر او را هدف بگیرد، زره روی خودِ زره‌ساز اجرا می‌شود. "
            "وارث می‌تواند توانایی او را به ارث ببرد."
        ),
        strategy="زره را روی نقش‌هایی بگذار که بیشترین احتمال هدف‌شدن دارند.",
    ),
    RoleDefinition(
        RoleCode.APOTHECARY,
        "عطار",
        RoleTeam.CITIZEN,
        "یک زهر و یک پادزهر دارد؛ می‌تواند شب‌ها زهر بریزد و با رأی شهر پادزهر بدهد.",
        objective="حذف مافیا با زهر و نجات شهر با پادزهر در لحظه درست.",
        night_ability=(
            "هر شب می‌تواند زهر خود را به یک نفر بدهد؛ گرداننده ورود زهر را اعلام می‌کند."
        ),
        day_ability=(
            "ابتدای شب بعد اگر فرد مسموم زنده باشد، رأی‌گیری شبانه برای استفاده از پادزهر "
            "انجام می‌شود (فرد مسموم حق رأی ندارد)؛ در صورت موافقت اکثریت، عطار تصمیم نهایی را می‌گیرد."
        ),
        timing="یک زهر و یک پادزهر در کل بازی.",
        limitations=(
            "اگر عطار در بازی نباشد نتیجه رأی‌گیری ملاک است. "
            "اگر جادوگر عطار را هدف بگیرد، عطار برای خودش زهر می‌ریزد و می‌تواند پادزهر را روی خودش استفاده کند."
        ),
        interactions="با جادوگر و دن (پادزهر دن) تعامل دارد؛ وارث می‌تواند نقش او را به ارث ببرد.",
        strategy="زهر را روی مظنون‌ترین فرد بریز و پادزهر را برای نجات نقش کلیدی نگه دار.",
    ),
    RoleDefinition(
        RoleCode.HEIR,
        "وارث",
        RoleTeam.CITIZEN,
        "شب معارفه یک نفر را انتخاب می‌کند و تا انتقال نقش، نامیرا است.",
        objective="به‌ارث‌بردن توانایی نقش‌های کلیدی شهر و بقا.",
        night_ability="شب معارفه یک بازیکن را انتخاب می‌کند؛ تا زمانی که نقش او منتقل نشده، نامیرا است.",
        timing="انتخاب یک‌باره در شب معارفه.",
        limitations=(
            "فقط اگر فرد انتخاب‌شده «کارآگاه»، «زره‌ساز» یا «عطار» باشد و از بازی خارج شود، "
            "توانایی او را به ارث می‌برد؛ در غیر این صورت فقط نامیرایی‌اش را از دست می‌دهد. "
            "وارث قابل سوداگری نیست."
        ),
        interactions="اگر نقش کدخدا را بگیرد، لینک‌های قبلی کدخدا منتقل نمی‌شوند.",
        strategy="کسی را انتخاب کن که احتمال زنده‌ماندن و ارزش نقشش بالاست.",
    ),
    RoleDefinition(
        RoleCode.KADKHODA,
        "کدخدا",
        RoleTeam.CITIZEN,
        "می‌تواند در طول بازی دو نفر را به لینک شهر اضافه کند و آن‌ها را بشناسد.",
        objective="ساختن شبکه‌ای مطمئن از شهروندان برای شکست مافیا.",
        night_ability="در طول بازی می‌تواند دو نفر را بیدار کند و به لینک شهر اضافه کند.",
        timing="در مجموع دو بار در کل بازی.",
        limitations=(
            "اگر مافیایی (به‌جز خبرچین) را بیدار کند، خودش و لینک‌هایش حذف می‌شوند. "
            "افراد لینک‌شده همدیگر را نمی‌شناسند. اگر لینک او سوداگری شود، دیگر توان استفاده از آن لینک را ندارد."
        ),
        interactions=(
            "خبرچین را «شهروند» می‌بیند. اگر جادوگر او را هدف بگیرد یکی از توانایی‌هایش از بین می‌رود. "
            "اگر وارث نقش او را بگیرد، لینک‌های قبلی منتقل نمی‌شوند."
        ),
        strategy="با احتیاط بیدار کن؛ یک اشتباه روی مافیا کل شبکه‌ات را نابود می‌کند.",
    ),
    RoleDefinition(
        RoleCode.CITY_TRUSTED,
        "معتمد شهر",
        RoleTeam.CITIZEN,
        "با رأی شهر در پایان روز اول انتخاب می‌شود و تفنگ کاپو در اختیارش قرار می‌گیرد.",
        objective="استفاده درست از تفنگ کاپو برای حذف مافیا.",
        day_ability=(
            "در پایان روز اول با بیشترین رأی به‌عنوان معتمد انتخاب می‌شود؛ سپس در شب موقت، "
            "دن تعیین می‌کند گلوله‌های تفنگ او جنگی یا مشقی باشند و معتمد می‌تواند شلیک کند."
        ),
        timing="یک‌بار در ابتدای بازی (شب موقت پس از روز اول).",
        limitations=(
            "می‌تواند تیر اول را هوایی بزند؛ اگر تیر اول مشقی باشد تیر دوم مجاز است. "
            "نمی‌تواند دو بار به یک نفر شلیک کند. اگر تیر جنگی شلیک شود، هدف پس از وصیت خارج می‌شود."
        ),
        interactions="اگر دن با تیر جنگی کشته شود، پادزهر دن به جادوگر منتقل می‌شود.",
        strategy="با دقت به بحث‌های روز اول گوش بده تا اگر گلوله جنگی گرفتی هدف را درست بزنی.",
    ),
    # --- Mafia (مافیا) ---
    RoleDefinition(
        RoleCode.GODFATHER,
